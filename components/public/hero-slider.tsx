...
export function HeroSlider({ slides }: { slides: HeroSlide[] }) {
  ...
  useEffect(() => { if (!usableSlides.length) return; const timer = setInterval(() => setActive((current) => (current + 1) % usableSlides.length), 5000); return () => clearInterval(timer); }, [usableSlides.length]);
